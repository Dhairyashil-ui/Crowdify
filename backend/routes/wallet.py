...
async def record_x402_usage(organization_id: str, features: list[str]):
    """
    Record an x402 programmatic API payment in the transaction ledger.
    Does NOT deduct from internal credits (balance is unaffected).
    """
    if _db is None:
        return
    try:
        ...
        await _db.wallet_transactions.insert_one({
            "organization_id": organization_id,
            "type":            "X402_API_PAYMENT",
            "source":          "PROGRAMMATIC",
            "feature":         ",".join(features),
            "amount_inr":      None,
            "credits":         0,  # Does not affect internal credit balance
            "status":          "SUCCESS",
            "reference_id":    "algorand_tx_pending",
            "created_at":      now,
        })
    except Exception as e:
        logger.error(f"[BILLING] Error recording x402 usage: {e}")
